...
describe("Oracle plugin frame unit tests", function () {
  ...
  describe("admin check", function () {
    it("only admin can call the setters", async function () {
      ...
      // setOracle
      await expect(this.oracleBasement.connect(this.signers[2]).setOracle(addr0000, addr0000, 0)).to.be.revertedWith(
        "Ownable: caller is not the owner",
      );

      // enableOracle
      await expect(this.oracleBasement.connect(this.signers[2]).enableOracle(addr0000, 0, false)).to.be.revertedWith(
        "Ownable: caller is not the owner",
      );
  ...
  describe("token config", function () {
    describe("add single token config", function () {
      ...
      it("token config added successfully & events check", async function () {
        ...
        await expect(result)
          ...
          .withArgs(addr1111, addr1111, addr1111, addr1111);
      });
    });
    ...
    describe("batch add token configs", function () {
      ...
      it("token config added successfully & data check", async function () {
        ...
        expect((await this.oracleBasement.getTokenConfig(addr1111)).oracles[0]).to.equal(addr1111);
        expect((await this.oracleBasement.getTokenConfig(getSimpleAddress(3))).oracles[1]).to.equal(
          getSimpleAddress(2),
        );
        expect((await this.oracleBasement.getTokenConfig(getSimpleAddress(3))).enableFlagsForOracles[0]).to.equal(true);
        // non exist config
        expect((await this.oracleBasement.getTokenConfig(getSimpleAddress(8))).enableFlagsForOracles[0]).to.equal(
          false,
        );
      });
    });
  ...
  describe("change oracle", function () {
    ...
    describe("set oracle", function () {
      it("null check", async function () {
        ...
        await expect(this.oracleBasement.setOracle(addr1111, addr0000, 0)).to.be.revertedWith(
      ...
      it("existance check", async function () {
        await expect(this.oracleBasement.setOracle(getSimpleAddress(2), addr1111, 0)).to.be.revertedWith(
      ...
      it("oracle set successfully & data check", async function () {
        ...
        expect((await this.oracleBasement.getTokenConfig(addr1111)).oracles).to.eql([
          addr1111,
          getSimpleAddress(2),
          addr0000,
        ]);
